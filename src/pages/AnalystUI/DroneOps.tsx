                ...
                <div key={d.id} className="group relative p-5 rounded-2xl bg-background border border-border/50 hover:border-sky-500/50 hover:shadow-[0_0_20px_-10px_rgba(14,165,233,0.3)] transition-all overflow-hidden">
                  ...
                  <div className="space-y-3 relative z-10">
                    ...
                    <div className="flex items-center justify-between pt-2 gap-2 flex-wrap">
                      <div className={`text-xs flex items-center gap-1.5 flex-wrap ${isInProgress ? 'text-red-400 font-medium' : 'text-muted-foreground'}`}>
                        <MapPin className="w-3 h-3 flex-shrink-0" />
                        <span className="whitespace-nowrap">{displayLat && displayLon ? `${displayLat.toFixed(4)}, ${displayLon.toFixed(4)}` : 'Position Unknown'}</span>
                        
                        {displayLat && displayLon && isInProgress && (
                           <Dialog>
                             <DialogTrigger asChild>
                               <button className="hover:bg-red-500/20 px-2 py-0.5 rounded border border-red-500/30 transition-colors text-[10px] text-red-400 whitespace-nowrap flex-shrink-0">View Tracking</button>
                             </DialogTrigger>
